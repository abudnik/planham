...
We obtained the following results during above-mentioned tests: algorithm execution time is proportional to the number of faces for most instances of graphs, while approximately 0.02% of the remaining instances of graphs are processed with exponential execution time. Next, we can evaluate the constant C in the search algorithm with `O(C^F)` complexity, where F - number of faces: `C = num_iter ^ (1/F)`, where num_iter - the obtained number of iterations for some input graph. We obtained the maximum value of `C ≅ 1.3803`. Thus, a rough estimate of the algorithm complexity is `O(1.3803^F)` in the worst case. <br />
